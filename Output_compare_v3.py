...
def compare_csv_files(file_pair: Tuple[str, str, int]) -> Tuple[pd.DataFrame, int, Dict]:
    ...
    without_zone_csv, with_zone_csv, part_num = file_pair
    
    print(f"  [Part {part_num}] Starting comparison...")
    
    # Read both CSVs with low_memory=False and dtype as object to preserve data
    df_without = pd.read_csv(without_zone_csv, keep_default_na=False, low_memory=False, dtype=str)
    df_with = pd.read_csv(with_zone_csv, keep_default_na=False, low_memory=False, dtype=str)
    
    # Find ID and description columns
    id_col = None
    desc_col = None
    ...
    for idx in range(max(len(df_without), len(df_with))):
        if idx >= len(df_without):
            ...
            with_output_str = str(df_with.iloc[idx].get('outputs_ner', ''))
            ...
            without_output_str = str(df_without.iloc[idx].get('outputs_ner', ''))
            ...
            without_output_raw = df_without.iloc[idx].get('outputs_ner', '')
            with_output_raw = df_with.iloc[idx].get('outputs_ner', '')
            
            # Parse for comparison
            output_without = parse_outputs_ner(without_output_raw)
            output_with = parse_outputs_ner(with_output_raw)
            
            is_match, comment = compare_outputs(output_without, output_with)
            
            # CRITICAL FIX: Store as string, not as parsed object
            # This prevents pandas from converting to empty during concat
            without_output_str = str(without_output_raw) if without_output_raw else "[]"
            with_output_str = str(with_output_raw) if with_output_raw else "[]"
...
def compare_all_parts(output_dir: str, use_multiprocessing: bool = True):
    ...
    file_pairs = [
        (without_zone_files[i], with_zone_files[i], i+1)
        for i in range(num_pairs)
    ]
    
    print(f"\n{'='*80}")
    if use_multiprocessing:
        # Determine number of workers (use 75% of available cores)
        ...
        with ProcessPoolExecutor(max_workers=num_workers) as executor:
            ...
            for future in as_completed(future_to_part):
                ...
                try:
                    ...
                    part_comparison_file = os.path.join(comparison_folder, f"comparison_part_{part_num:03d}.csv")
                    comparison_df.to_csv(part_comparison_file, index=False)
    ...
    # Combine all comparisons in correct order
    print(f"\n{'='*80}")
    ...
    # Save combined comparison with proper dtype to preserve strings
...
def main():
    ...
    # Set to False if multiprocessing causes issue
